<template>
  <!-- 결제 성공 시 -->
  <section v-if="confirmed" style="margin-top: 50px">
    <div
      class="box_section"
    ...
    >
      ...
      >
        <button class="button p-grid-col5, margin: center" @click="purchase">
          등기 정보 업데이트
        </button>
      </div>
    </div>
  ...
  methods: {
    ...
    purchase() {
      try {
        ...
        // }
        // console.log(this.jsonData.balanceAmount);
        let note =
          "소유권 이전_" +
          this.jsonData.easyPay.provider +
          "_" +
          this.jsonData.approvedAt +
          "_" +
          this.jsonData.balanceAmount +
          this.jsonData.currency;
        DUBU.methods
          .buyItem(
